<?php
// BOF ZenExpert optimized images - admin
?>
<?php
...
function zx_optimized_images($src, $width, $height) {
    ...
    if (!file_exists(DIR_FS_CATALOG.DIR_IMAGES_OPTIMIZED . $name[0])) {
        mkdir(DIR_FS_CATALOG.DIR_IMAGES_OPTIMIZED . $name[0], 0755, true);
    }
    ...
    $requested_image = DIR_FS_CATALOG.DIR_IMAGES_OPTIMIZED. $name[0].'/'.$name .'-'.$width.'x'.$height.'.'.$ext;
    if(!file_exists($requested_image)) {
        $resize = true;
    }

    // set target file name (first part)
    $target_base = DIR_FS_CATALOG.DIR_IMAGES_OPTIMIZED . $name[0].'/'.$name.'-';

   if($resize) {
       require_once(DIR_FS_CATALOG.DIR_WS_CLASSES.'vendors/imageMagician/php_image_magician.php');
...
function resmushit($file) {
    ...
    $messageStack->add('image successfully optimized: '.$file, 'success');
